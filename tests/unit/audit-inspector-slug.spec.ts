...
describe('writeAuditLogWithSlug — Sprint B-3', () => {
    ...
    it('writes inspector_slug on inspection.created', async () => {
        await writeAuditLogWithSlug(sqlite, { tenantId: TENANT, actorUserId: USER, action: 'inspection.created', entityType: 'inspection', entityId: 'i-1' });
    ...
    it('leaves inspector_slug NULL on user.login.success (not in allowlist)', async () => {
        await writeAuditLogWithSlug(sqlite, { tenantId: TENANT, actorUserId: USER, action: 'user.login.success', entityType: 'user', entityId: USER });
        const rows = await testDb.select().from(schema.auditLogs).all();
        expect(rows.length).toBe(1);
    ...
    it('handles inspector with no slug gracefully (NULL slug)', async () => {
        ...
        await writeAuditLogWithSlug(sqlite, { tenantId: TENANT, actorUserId: USER, action: 'inspection.created', entityType: 'inspection', entityId: 'i-2' });
        const rows = await testDb.select().from(schema.auditLogs).all();
        expect(rows[0]?.inspectorSlug).toBeNull();
    });
    ...
    it('writes inspector_slug for all 6 allowlist events', async () => {
        ...
        for (const action of allowlist) {
            await writeAuditLogWithSlug(sqlite, { tenantId: TENANT, actorUserId: USER, action, entityType: 'inspection', entityId: 'i-' + action });
        }
        const rows = await testDb.select().from(schema.auditLogs).all();
